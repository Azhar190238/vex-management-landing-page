...
interface SplitTextProps {
  ...
  easing?: (t: number) => number; // easing must be a function
  textAlign?: "left" | "center" | "right" | "justify";
  heighlightsword?: number[];
  heighlightclass?: string;
}
...
const SplitText: React.FC<SplitTextProps> = ({
  ...
  easing = easings.easeOutCubic, // ✅ function, not string
  textAlign = "center",
  heighlightsword = [],
  heighlightclass = "",
}) => {
  ...
  const springs = useSprings(
    ...
    letters.map((_, i) => ({
      ...
      delay: i * delay,
      config: { easing }, // ✅ easing is now a function
    }))
  );
  ...
  return (
    ...
    >
      {words.map((word, wordIndex) => {
        ...
        const isHighlighted = heighlightsword.includes(wordIndex + 1);
